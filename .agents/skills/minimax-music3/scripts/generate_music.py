#!/usr/bin/env python3
"""
Generate music, songs, or instrumentals using MiniMax Music3 on ComfyUI.
Usage:
    python generate_music.py --caption "..." --lyrics "..." [--duration 30] [--seed 42] [--format all] [--host http://<NODE_IP>:8188]
"""
...
OUTPUT_DIR = os.path.join(os.environ.get("COMFY_DIR", os.path.expanduser("~/ComfyUI-music3")), "output")
...
def main():
    ...
    if not check_server(host):
        ...
        print(f"[HINT] If running across cluster nodes, verify the node IP and network route (e.g., ping / curl {host}/system_stats).", file=sys.stderr)
        print(f"[HINT] If running locally on this node, ensure server is started via start_server.sh.", file=sys.stderr)
        sys.exit(1)
        ...
        try:
            ...
            if prompt_id in hist:
                ...
                if audio_list:
                    ...
                    flac_path = os.path.join(OUTPUT_DIR, subfolder, filename)
                break
        except Exception as e:
            print(f"[WARN] Error polling history from {host}: {e}")

    elapsed = time.time() - start_t
    print(f"[SUCCESS] Generation finished in {elapsed:.1f}s!")
    ...
    if flac_path and os.path.exists(flac_path):
        ...
        converter_script = os.path.join(script_dir, "convert_media.py")
